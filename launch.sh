...
if ! python -c 'import tkinter' 2>/dev/null || ! command -v startx >/dev/null || ! command -v openbox >/dev/null; then
  printf 'Preparing the graphical installer in the live environment. Connect to the internet first.\n'
  pacman -Sy --needed --noconfirm python tk xorg-server xorg-xinit xorg-xauth xorg-xkbcomp xorg-setxkbmap openbox ttf-dejavu git
fi
if [[ -n ${DISPLAY:-} ]]; then
